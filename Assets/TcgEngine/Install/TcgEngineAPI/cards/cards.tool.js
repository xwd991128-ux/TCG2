...
{
    // Use all cards for special_legendary pack
    var pack_cards = [];
    if(pack.tid === 'special_legendary')
    {
        // For special_legendary pack, use all cards
        pack_cards = await CardModel.getAll();
        console.log('special_legendary pack: found ' + pack_cards.length + ' cards');
    }
    else
    {
        // For other packs, use pack-specific cards
        var packId = pack.tid;
        pack_cards = await CardModel.getByPack(packId);
        console.log(packId + ' pack: found ' + pack_cards.length + ' cards');
    }
    ...
    for(var i=0; i<pack.cards; i++)
    {
        if(pack.random)
        {
            ...
            if(pack.tid === 'special_legendary' && i === pack.cards - 1)
            {
                rarity_tid = 'mythic'; // Force mythic rarity for last card
            }
            
            console.log('Card ' + i + ': looking for rarity=' + rarity_tid);
            
            var variant_tid = CardTool.getRandomVariant(pack);
            var rarity_cards = CardTool.getCardArray(pack_cards, rarity_tid);
            console.log('Found ' + rarity_cards.length + ' cards with rarity=' + rarity_tid);
            ...
            if(card)
            {
                ...
                cards.push(cardQ);
                console.log('Added card: ' + card.tid);
            }
            else
            {
                console.log('No card found for rarity=' + rarity_tid);
            }
        }
    ...
    console.log('Total cards in pack: ' + cards.length);
    return cards;
};
