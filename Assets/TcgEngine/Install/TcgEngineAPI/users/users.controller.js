...
exports.GoldBattleReward = async(req, res) => 
{
    var userId = req.params.userId;
    ...
    var updatedUser = await UserModel.save(user, ["coins"]);
    if (!updatedUser) return res.status(500).send({ error: "Error updating user: " + userId });

    var activityData = {coins: coins, user: user.username};
    var act = await Activity.LogActivity("gold_battle_reward", req.jwt.username, activityData);
    if (!act) return res.status(500).send({ error: "Failed to log activity!!" });

    return res.status(200).send({coins: user.coins});
};
